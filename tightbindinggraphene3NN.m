...
E=ones(4.732*N,2);
Vector=ones(2*4.732*N,2);
G=[0,0];
K=[(b1(1)-b2(1))/3,(b1(2)-b2(2))/3];
M=[b1(1)/2,b1(2)/2];
%--k points from M to Gamma in reciprocal space
for k=1:+1:1.732*N
    [Vector(2*k-1:2*k,1:2),E(k,1:2)]=Hmatrix(M(1)+(G(1)-M(1))/(1.732*N)*k,M(2)+(G(2)-M(2))/(1.732*N)*k);
end
%--k points from Gamma to K in reciprocal space
for k=1.732*N+1:+1:3.732*N
    [Vector(2*k-1:2*k,1:2),E(k,1:2)]=Hmatrix(G(1)+(K(1)-G(1))/(2*N)*(k-1.732*N),G(2)+(K(2)-G(2))/(2*N)*(k-1.732*N));
end
%--k points from K to M in reciprocal space
for k=3.732*N+1:+1:4.732*N
    [Vector(2*k-1:2*k,1:2),E(k,1:2)]=Hmatrix(K(1)+(M(1)-K(1))/N*(k-3.732*N),K(2)+(M(2)-K(2))/N*(k-3.732*N));
end


figure (1)
for k=1:+1:4.732*N
    plot(k,real(E(k,1:2)+3.2),'r.','MarkerSize',15);hold on;
end
...
title('E-k diagram with Tight binding (3NN) in graphene');
...
function [Eigenvector,Eigenvalue]=Hmatrix(kx,ky) 
    data=importdata("wannier90_hr_to_3NN.dat");
    dmatrix=ones(26,4);
    for m =1:length(data(:,1))
        [dmatrix(m,1),dmatrix(m,2),dmatrix(m,3),dmatrix(m,4)]=getdlengthandvector(data(m,1),data(m,2),data(m,3),data(m,4),data(m,5));
